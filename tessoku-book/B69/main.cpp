...
int main() {
    ...
    const ll HOURS = 24;
    const ll MAX_WORK_TIME = 10;
    // 社員数 + 一日の時間 + 始点 & 終点の合計が頂点数
    const ll GRAPH_SIZE = N + HOURS + 2;
    const ll GRAPH_START_IDX = N + HOURS;
    const ll GRAPH_END_IDX = N + HOURS + 1;
    // グラフを表現する隣接リスト
    ...
    REP(i, N) {
        // 1.始点→i番目の社員の辺を張る
        create_edge(MAX_WORK_TIME, GRAPH_START_IDX, i);
    }
    REP(i, HOURS) {
        // 2.終点→j番目の時間帯の辺を張る
        create_edge(M, N + i, GRAPH_END_IDX);
    }
    ...
    auto min_can_work = M * HOURS;
    auto can_work = sum_flow >= min_can_work;
    cout << (can_work ? "Yes" : "No") << endl;
}
